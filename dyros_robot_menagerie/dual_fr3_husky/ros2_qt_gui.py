...
class Ros2QtGui(Node, QWidget):
    def __init__(self, topic_ns: str = 'dual_fr3_husky_controller'):
        Node.__init__(self, 'ros2_qt_gui')
        QWidget.__init__(self)
        self.setWindowTitle('Dual FR3-Husky Controller GUI')
        ...
        self.base_vel_sub = self.create_subscription(
            Twist, f'{self.ns}/base_vel', self.base_vel_callback, 10
        )
        self.base_pose_sub = self.create_subscription(
            PoseStamped, f'{self.ns}/base_pose', self.base_pose_callback, 10
        )
    ...
    def _build_right_feedbacks(self):
        ...
        # Joint states (wheels + 7 right joints + 7 left joints)
...
def main():
    ...
    gui = Ros2QtGui(topic_ns='dual_fr3_husky_controller')
    gui.show()
    try:
        exit_code = app.exec_()
    finally:
